/* ============================================================
   ...
   const getNowCST = () => new Date(new Date().toLocaleString('en-US',{timeZone:CONFIG.twitch.timezone}));
   ...
   function initBackground() {
     ...
     img.onerror = () => { el.style.opacity='1'; };
     img.src = src;
   }
   ...
   function applyLiveState(live) {
     ...
     if (pill) pill.classList.toggle('hidden', !live);
   
     const badge = $('#hero-live-badge');
     if (badge) {
       badge.className = live ? 'hero-status-badge live' : 'hero-status-badge offline';
       badge.querySelector('.badge-text').textContent = live ? 'LIVE NOW' : 'OFFLINE';
     }
     ...
     mountEmbed(live);
     if (live) setTimeout(openPopup, CONFIG.popupDelay);
   
     console.log(`%cDarkSalxm%c ${live?'🔴 LIVE':'⚫ OFFLINE'}`,
   ...
   function youtubeEmbedSrc(videoId='') {
     ...
     if (videoId) return `${base}/${videoId}?rel=0&modestbranding=1&color=red`;
   ...
   async function mountEmbed(live) {
     ...
     if (live) {
       ...
       if (dot)  { dot.className = 'embed-dot vod'; }
     }
   
     if (label)   label.textContent = labelText;
     if (extLink) { extLink.href = href; extLink.textContent = live ? 'OPEN TWITCH ↗' : 'OPEN YOUTUBE ↗'; }
   ...
   function openPopup() {
     ...
     if (iframe && !iframe.src) iframe.src = twitchEmbedSrc();
   ...
   function renderSchedule() {
     ...
     grid.innerHTML = SCHEDULE.map((s, i) => {
       ...
       const classes = ['sched-card', s.stream&&'stream-day', today&&'today'].filter(Boolean).join(' ');
       return `
         <div class="${classes}">
   ...
   function initTypewriter() {
     ...
     function tick() {
       ...
       if (!del) {
         ...
         if (c === phrase.length) { del=true; setTimeout(tick,2400); return; }
         setTimeout(tick,55);
       } else {
         el.textContent = phrase.slice(0,--c);
     ...
     $$('.copy-email').forEach(el=>{
       el.addEventListener('click',()=>{
         navigator.clipboard.writeText(el.dataset.email||el.textContent.trim())
           ...
           }).catch(()=>location.href=`mailto:${el.dataset.email||el.textContent.trim()}`);
       });
     });
   }
